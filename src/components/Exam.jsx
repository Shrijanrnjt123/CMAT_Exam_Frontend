...
import { question_view, send_Result } from "../api/Api.jsx";
...
const Exam = () => {
  ...
  const showLine = [
    ...
    "Direction: The passage given below s followed by questions based on its contents. After reading the passage, choose the best answer to each question.",
  ...
  }, [timeLeft, showPopup, examSubmitted]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const requiredModel = data.find((item) => item._id === id);
      if (requiredModel) {
  ...
  const submitExam = () => {
    ...
    sendResult();
    setIsTotal(true)
    setExamSubmitted(true); // Exam submitted
  };
    ...
    <div className={styles.container}>
      ...
      <div className={styles.timerSubmit}>
        ...
        <button
          ...
          disabled={isDisabled} // Disable the button if the exam is submitted
        >
          Submit
        </button>
      </div>
